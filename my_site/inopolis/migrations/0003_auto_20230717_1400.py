...
def default_films(apps, schema_editor):
    ...
    IS = Films.objects.create(title="Infinite Storm", description=description1, rating="R", director="Malgorzata Shumowska", producer="Michal Englert", writer="Taya Gagne", release_date_theaters=datetime.date(2022,3,25), release_date_streaming=datetime.date(2023, 7, 17), data='inopolis/video/beskonecnaya_burya.mp4')
    IS.genre.create(name="Drama")
    R = Films.objects.create(title="Rambo IV", description=description2, rating="G", director="Sylvester Stallone", producer="Kevin King Templeton, Avi Lerner", writer="Art Monterastelli, Sylvester Stallone, David Morell", release_date_theaters=datetime.date(2008, 1, 23), release_date_streaming=datetime.date(2023, 7, 17), data='inopolis/video/rambo4.mp4')
    R.genre.create(name="Action")
    S = Films.objects.create(title="Snatch", description=description3, rating="PG", director="Guy Ritchie", producer="Matthew Vaughn, Michael Dreyer, Stephen Marks", writer="Mel Brooks, Thomas Mian", release_date_theaters=datetime.date(2005, 12, 16), release_date_streaming=datetime.date(2023, 7, 17), data='inopolis/video/snatch.mp4')
    S.genre.create(name='Comedy')

class Migration(migrations.Migration):

    dependencies = [
        ('inopolis', '0002_alter_film_data'),
    ]
